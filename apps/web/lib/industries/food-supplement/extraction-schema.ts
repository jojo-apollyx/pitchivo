...
7. CRITICAL DATA TYPE REQUIREMENTS:
   - ALL field values MUST be simple, plain types: string, number, boolean, or arrays of strings/numbers
   - NEVER output complex objects, nested objects, or structured data as field values
   - If information is in a complex format (e.g., sections with subsections, tables, structured lists), convert it to plain text
   - For example: If "First Aid Measures" contains multiple subsections, combine them into a single plain text string like "Inhalation: Remove to fresh air. Skin contact: Wash with soap and water. Ingestion: Seek medical attention."
   - For structured information, extract the key information and present it as readable plain text
8. CRITICAL: Only output fields with actual meaningful data. DO NOT output fields if:
...
9. If a field is not found in the document or contains only placeholder/unavailable indicators, completely omit that field from the output (do not include it with null, empty string, or placeholder values)
